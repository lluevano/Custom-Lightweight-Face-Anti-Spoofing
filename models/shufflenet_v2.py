...
from .model_tools import AntiSpoofModel, get_activation
...
class BasicUnit(nn.Module):
    def __init__(self, inplanes, outplanes, c_tag=0.5, activation="relu", SE=False, residual=False, groups=2):
...
class DownsampleUnit(nn.Module):
    def __init__(self, inplanes, c_tag=0.5, activation="relu", groups=2):
        ...
        self.activation = get_activation(name=activation, **(dict(inp=inplanes, oup=inplanes)))
...
class ShuffleNetV2(AntiSpoofModel):
    ...
    def __init__(self, scale=1.0, c_tag=0.5, activation="relu",
        ...
        self.conv_last = nn.Sequential(
            nn.Conv2d(self.c[-2], self.c[-1], kernel_size=1, bias=False),
            nn.BatchNorm2d(self.c[-1]),
            get_activation(name=activation, **(dict(inp=self.c[-1], oup=self.c[-1]))))
        self.bn_last = nn.BatchNorm2d(self.c[-1])
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.spoofer = nn.Linear(self.c[-1], self.num_classes)
        self.init_params()
...
def get_ShuffleNetV2_model(activation="relu", **kwargs):
    
    return ShuffleNetV2(activation=activation,
                          SE=True, residual=True, **kwargs)
