...
def prepare_test_set():
    metadata_iter = glob2.iglob("./test-v2/data/*.txt")
    ...
    for metadata in metadata_iter:
        with open(metadata, 'r') as f:
            ...
            if not cv2.imwrite(os.path.join(".","test-v2","cropped_data",img_name),img_crop):
                print(f"Cannot save {img_name} in cropped_data")
                exit(-1)
            ...
            if not cv2.imwrite(os.path.join(".","test-v2","norm_crop",img_name),cv2.cvtColor(img_norm_crop, cv2.COLOR_RGB2BGR)):
                print(f"Cannot save {img_name} in norm_crop")
                exit(-1)
             
            dev_list += (os.path.join("test-v2","data",img_name)+'\n')
            crop_list += (os.path.join("test-v2","cropped_data",img_name)+'\n')
            norm_crop_list += (os.path.join("test-v2","norm_crop",img_name)+'\n')
            
    with open(os.path.join(".","test-v2","test-v2_data.txt"),'w') as f_dev_data:
                f_dev_data.write(dev_list.strip())
    
    with open(os.path.join(".","test-v2","cropped_data.txt"),'w') as f_cropped_data:
                f_cropped_data.write(crop_list.strip())
                
    with open(os.path.join(".","test-v2","norm_crop.txt"),'w') as f_norm_crop:
                f_norm_crop.write(norm_crop_list.strip())
